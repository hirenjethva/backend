...
export const updateProduct = catchAsyncErrors(async (req, res) => {
  ...
  // Check if stock updates are provided
  if (req.body.stock) {
    // Validate stock format
    const stockIsValid = Array.isArray(req.body.stock) && 
                         req.body.stock.every((item) => 
                           item.size && typeof item.size === "string" &&
                           typeof item.quantity === "number"
                         );

    if (!stockIsValid) {
      return next(new ErrorHandler("Invalid stock format", 400));
    }
  }
...
export const uploadProductImages = catchAsyncErrors(async (req, res) => {
  ...
  const uploader = async (image) => upload_file(image, "Gusai/products");
