...
import { getResetPasswordTemplate, getVerificationEmailTemplate } from "../utils/emailTemplates.js";
...
export const registerUser = catchAsyncErrors(async (req, res, next) => {
  ...
  const message = getVerificationEmailTemplate(name, verificationUrl);
...
export const verifyEmail = catchAsyncErrors(async (req, res, next) => {
  try{
    ...
    if (!token) {
      return next(new ErrorHandler("Invalid or missing verification token", 400));
    }
...
export const uploadAvatar = catchAsyncErrors(async (req, res, next) => {
  ...
  const avatarResponse = await upload_file(filePath, "Gusai/avatars");
...
export const forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    await emailService.sendEmail(
        user.email,                    // to
        "Gusai Password Recovery",         // subject
        message                   // html content
    );
